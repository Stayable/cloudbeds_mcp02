/**
 * Guest-facing door-code emails (Resend REST API, same transport as email.ts).
 * Four lifecycle messages, all built by PURE functions (buildGuestEmail) so the
 * copy/markup is unit-testable; sendGuestEmail is the thin I/O wrapper.
 *
 ...
 *
 * "rotated" here means the GUEST's own code value changed (a re-issue / resend),
