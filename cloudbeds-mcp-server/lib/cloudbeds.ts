/**
 * Thin client for the Cloudbeds PMS API (v1.2). Identical contract to the
 * stdio `cloudbeds-mcp` package — Vercel deploys a single root directory and
 * cannot import from a sibling folder, so the client lives here too.
 *
 * Auth: self-service API key (`x-api-key`) or OAuth bearer token.
 * Docs/spec: https://github.com/cloudbeds/openapi-specs (pms-v1.2-openapi.yaml)
 */
...
export class CloudbedsClient {
  ...
  /** Build a single client from environment variables. */
  static fromEnv(): CloudbedsClient {
    return new CloudbedsClient({
      apiKey: process.env.CLOUDBEDS_API_KEY,
      accessToken: process.env.CLOUDBEDS_ACCESS_TOKEN,
      baseUrl: (
 ...
 * `CLOUDBEDS_API_KEY_<propertyID>` (e.g. CLOUDBEDS_API_KEY_812). A bare
